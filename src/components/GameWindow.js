...
function GameWindow({ location, click, setLocation, setText, setOptions, setTarget, 
                    map, setMap, player }) {
    const [translateX, setTranslateX] = useState(0.5);
    const [translateY, setTranslateY] = useState(0);
    const [playerCoords, setPlayerCoords] = useState(player.coords);

    const [centerX, setCenterX] = useState(map[0].length / 2);
    const [centerY, setCenterY] = useState(map.length / 2)

    var translations = 'translate('+(centerX-translateX)+'ch,'+(centerY-translateY)+'em)';
    const test = {
    ...
    return (
        <div className="container-fluid text-center game">
            <div className="row content gameWindow">
                <div className="col-sm-2 sidenav">
                </div>
                <div className="col-sm-8 mapContainer ">
                    <Map map={map} test={test} />
                    <Player player={player} translateX={translateX} 
